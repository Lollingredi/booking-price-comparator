"""
Expedia Playwright-based price scraper.

Uses the same thread-isolation approach as BookingProvider.
hotel_key format: same Booking.com slug (used to build the Expedia search URL).
"""
import asyncio
import logging
import random
...
async def _do_fetch_rates_expedia(
    hotel_name: str, check_in: str, check_out: str, proxy: str | None
) -> list[RateResult]:
    """Scrape Expedia search results for a hotel name."""
    try:
    ...
    url = (
        f"https://www.expedia.it/Hotel-Search?"
        f"destination={query.replace(' ', '%20')}"
        f"&startDate={check_in}&endDate={check_out}"
        f"&rooms=1&adults=2&currency=EUR"
    )
    logger.info("Scraping Expedia: query=%s  %s→%s", query, check_in, check_out)
    ...
    if not prices:
        logger.warning("Expedia: no prices found for query=%s", query)
        return []

    min_price = round(min(prices), 2)
    logger.info("Expedia min price for %s: €%.2f", query, min_price)
    return [RateResult(ota_code="expedia", ota_name="Expedia", price=min_price, currency="EUR")]


class ExpediaProvider(RateProvider):
    """Fetches hotel prices from Expedia using Playwright."""
    ...
    async def fetch_rates(self, hotel_key: str, check_in: str, check_out: str) -> list[RateResult]:
        ...
        return await asyncio.to_thread(
            _run_in_own_loop,
            lambda: _do_fetch_rates_expedia(hotel_key, check_in, check_out, proxy),
        )

    async def search_hotel(self, query: str) -> list[HotelSearchResult]:
        # Expedia search not implemented — use Booking.com search
        return []
